...
<html>
	...
	<body>
	  <div id="include-div">
		<h3>安排问卷答题</h3>
		<form action="<%=basePath %>permission/select_fpsj.action" name="fpqx">
		...
		<table class="employee-list">
		  <tr>
			<th class="checkbox">
			  <input type="checkbox" id="xuanzhong" onclick="isOr()">全选
			</th>
			<th class="name">姓名</th>
		  </tr>
		...
		<%session.setAttribute("person_info",null);}//end of for%>
		
		</div>
		<div style="float:left;width:40px;"></div>
		<div style="float:left;width:200px;border-left:1px solid gray">
		 <!-- 显示答题试卷 -->
		 <%List<Test_produce> list_dtqx=(List<Test_produce>)session.getAttribute("list_dtqx"); 
		if(list_dtqx!=null){
		%>
		<p><label>请选择试卷</label></p>
		<%for(int i=0;i<list_dtqx.size();i++){ %>
		<p>
			&nbsp;&nbsp;<input type="checkbox"  name="name2"
				value="<%=list_dtqx.get(i).getTp_id()%>">
		   <%=list_dtqx.get(i).getTp_title()%>
		</p>
			<%} %>
		...
		document.fpqx.action="<%=basePath%>permission/save_sj.action";
		fpqx.submit();
		}
