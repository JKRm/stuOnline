...
-->
			...
			pd = (Person_detail) request.getAttribute("pd");
			String imgpath = "../../studentOnline_source/community/portrait/";
		%>
		<form action="community/saveDetail.action" method="post" enctype="multipart/form-data">
		<p>
			用户ID：<%=pd.getPerson_id()%></p>
		<p>
			昵称：<input type="text" id="dv.nickname" name="dv.nickname" value="<%=pd.getPd_nickname()%>"></p>
		<p>
			邮箱：<input type="text" id="dv.email" name="dv.email" value="<%=pd.getPd_nickname()%>"></p>
		<p>
			头像：<img src="<%=imgpath+pd.getPd_photo() %>" width="54px" height="54px"/>
				 <a href="javascript:void(0)" onclick="addNewLine(this,'changeImg')">修改</a>
				 <br><a id="changeImg"></a>
