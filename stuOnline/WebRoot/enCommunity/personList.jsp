...
<html>
	...
	function topage(currentpage) {
			...
			location.href = "<%=basePath%>community/personSearch.action?currentpage=" + currentpage;
			form.submit();
	}
	</script>
	</head>
...
-->
			...
			<div id="content-wrap">
				...
				<div id="main">
					...
					<h2>
						<a href="index.html" title="">查询结果</a>
					</h2>
						
			

				<%List<Person_detail> pdList = new ArrayList<Person_detail>(); 
   pdList = (List<Person_detail>)request.getAttribute("pdList");
   if(pdList!=null){
   	 for(int i=0;i<pdList.size();i++){
   		Person_detail pd = new Person_detail();
   		pd = pdList.get(i);	
   %>
   <p><a href="<%=basePath %>community/homeDes.action?pid=<%=pd.getPerson_id()%>"><%=pd.getPd_nickname() %></a></p>
   <%} }else{%>
   <a>查无此人</a>
   <%} %>
